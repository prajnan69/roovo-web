...
import { reverseGeocodeDistrict } from '@/lib/googleMaps';
...
// Each card gets its own component (not inlined in the .map) so it can hold
// its own async district lookup — a hook can't live inside a loop body.
const RecentlyViewedCard: React.FC<{ listing: Listing; gradient: string[]; index: number; onOpen: () => void }> = ({ listing, gradient: g, index, onOpen }) => {
  const imageUrl = (() => {
  ...
  // Same field fallback chain as ListingCard — `city`/`place` aren't real
  // fields on the API shape (location lives under listing.location). When
  // none of the stored fields have data, fall back to a district-level
  // reverse-geocode from the listing's coordinates ("Bengaluru Urban",
  // "Uttara Kannada") instead of a state name or hiding the line.
  const storedLocation = (listing as any).place || listing.location?.city || (listing.location as any)?.address;
  const [fetchedDistrict, setFetchedDistrict] = useState<string | null>(null);

  useEffect(() => {
    if (storedLocation || fetchedDistrict) return;
    const lat = (listing as any).latitude || (listing.location as any)?.latitude;
    const lng = (listing as any).longitude || (listing.location as any)?.longitude;
    if (lat && lng) {
      reverseGeocodeDistrict(Number(lat), Number(lng))
        .then(setFetchedDistrict)
        .catch((err) => console.error('Failed to fetch district for recently-viewed card', err));
    }
  }, [listing, storedLocation, fetchedDistrict]);

  const locationLabel = storedLocation || fetchedDistrict;
...
const RecentlyViewedBanner: React.FC<RecentlyViewedBannerProps> = ({ listings }) => {
  ...
  return (
    <div style={{ padding: '4px 0 20px' }}>
      ...
      {/* Horizontal scrolling row. scrollPadding (not just padding) is needed
          alongside scroll-snap: WebKit (iOS WebView) snaps to the edge of the
          content box and ignores the row's own padding once you've scrolled
          and snapped back, so the left gap disappears — scroll-padding is the
          property scroll-snap itself respects. */}
      <div
      ...
      >
        {listingsToShow.map((listing, index) => (
          <RecentlyViewedCard
            ...
            index={index}
            onOpen={() => { triggerHaptic(); navigate(`/listing/${listing.id}`); }}
          />
        ))}
      </div>
    </div>
